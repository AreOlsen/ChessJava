...
public abstract class Piece {
   ...
   */
  public Piece(Board board, Position position, Side side) throws IllegalArgumentException {
    if (board == null) {
   ...
   *
   * @return List of moves piece can move to.
   */
  protected List<Move> getLegalMoves() {
    ...
    for (int row = 0; row < board.getHeight(); row++) {
      for (int col = 0; col < board.getWidth(); col++) {
        ...
        if (legalMoveEnd(position)) {
          positions.add(new Move(getPosition(), this, position, board.getPiece(position)));
        }
      }
    }
   ...
   */
  public boolean legalMoveEnd(Position end) {
    ...
    Optional<Piece> capturedPiece = board.getPiece(end);
